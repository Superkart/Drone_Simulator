...
void UCitySampleAnimInstance_Crowd_Head::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	//Workaround for bug when copying from a master pose that has a master pose with a different bone hierarchy
	if(SourceMeshComponent && SourceMeshComponent->MasterPoseComponent.IsValid())
	{
		USkeletalMeshComponent* MasterPoseComp = Cast<USkeletalMeshComponent>(SourceMeshComponent->MasterPoseComponent.Get());
		if(MasterPoseComp)
		{
			SourceMeshComponent = MasterPoseComp;
		}
	}
