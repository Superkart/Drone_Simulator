...
void ULoadingUtilLibrary::ApplyCustomPriorityLoading(const UObject* WorldContextObject, bool UseHighPriorityLoading, float MaxAsyncLoadingMilliSeconds, float MaxActorUpdateMilliSeconds, double MaxMassActorDestructionTimeLimit, double MaxMassActorSpawnTimeLimit)
{
	...
	GET_MASS_CONFIG_VALUE(DesiredActorDestructionTimeSlicePerTick) = MaxMassActorDestructionTimeLimit;
	GET_MASS_CONFIG_VALUE(DesiredActorSpawningTimeSlicePerTick) = MaxMassActorSpawnTimeLimit;
}
...
void ULoadingUtilLibrary::CaptureDefaultLoadingSettings()
{
	if (!HasCapturedDefaults)
	{
		...
		DefaultMassActorDestructionTimeLimit = GET_MASS_CONFIG_VALUE(DesiredActorDestructionTimeSlicePerTick);
		DefaultMassActorSpawnTimeLimit = GET_MASS_CONFIG_VALUE(DesiredActorSpawningTimeSlicePerTick);
