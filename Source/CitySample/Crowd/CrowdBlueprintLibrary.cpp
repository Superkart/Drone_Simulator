...
{
	if (!BaseComponent || !BaseComponent->SkeletalMesh || !AccessoryComponent)
	{
		return;
	}
...
{	
	if (!BaseComponent || !BaseComponent->SkeletalMesh || !SceneComponent)
	{
		return;
	}
...
{
	if (!SkinnedMeshComponent || !SkinnedMeshComponent->SkeletalMesh)
	{
		return FTransform::Identity;
	};

	// Get Ref Skeleton
	const FReferenceSkeleton& RefSkeleton = SkinnedMeshComponent->SkeletalMesh->GetRefSkeleton();
...
UMaterialInstanceDynamic* UCitySampleCrowdFunctionLibrary::CreateDynamicMaterialInstance(UMeshComponent* Component, const FName MaterialSlotName)
{
	...
	{
		if (!SkinnedComponent->SkeletalMesh)
		{
			return nullptr;
		}
		
		TArray<FSkeletalMaterial>& MeshMaterials = SkinnedComponent->SkeletalMesh->GetMaterials();
