...
void ACitySampleCrowdCharacter::UpdateMeshes(const FCrowdCharacterDefinition& CharacterDefinition)
{
	...
	for (uint8 MeshIdx = 0; MeshIdx < static_cast<uint8>(ECrowdMeshSlots::MAX); ++MeshIdx)
	{
		...
		if (MeshComponent && CharacterMeshes.IsValidIndex(MeshIdx))
		{
			...
			// 
			// We dont want a master pose on the base or the head
			if (MeshSlot == ECrowdMeshSlots::Base || MeshSlot == ECrowdMeshSlots::Head)
			{
				// There is a bug where the masterpose is getting serialized, let's set this to null just in case.
				MeshComponent->SetMasterPoseComponent(nullptr);
			}
			else
			{
				// Only set the master pose component if we have a valid mesh on the base mesh
				if (BaseMeshComponent && IsValid(BaseMeshComponent->SkeletalMesh))
				{
					MeshComponent->SetMasterPoseComponent(BaseMeshComponent);
				}
				else
				{
					MeshComponent->SetMasterPoseComponent(nullptr);	
...
void ACitySampleCrowdCharacter::OnGetOrSpawn(UMassEntitySubsystem* EntitySubsystem, const FMassEntityHandle MassAgent)
{
