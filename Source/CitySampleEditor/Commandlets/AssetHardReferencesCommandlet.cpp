...
{
	auto SerializeAssetInformation = [&](const FName& PackageName, const FName& AssetName, const FName& AssetClass, const FPackageReferenceInformation& ReferenceInformation) {
	...
	if (AssetsInFolderPtr)
	{
		...
		for (const FAssetData& AssetInFolder : AssetsInFolder)
		{
			...
			SerializeAssetInformation(AssetPackageName, AssetInFolder.AssetName, AssetInFolder.AssetClass, ReferenceInformation);
		}
	}
	else
	{
		for (const TPair<FName, FPackageReferenceInformation>& ReferencePair : ReferenceMap)
		{
			...
			SerializeAssetInformation(CurrentPackageName, FName(""), FName("Package"), CurrentReferenceInformation);
		}
	}
}
